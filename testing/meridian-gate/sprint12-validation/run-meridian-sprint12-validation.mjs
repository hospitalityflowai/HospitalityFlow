/**
 * Meridian Gate — Sprint 12 room-token identity validation (TEST ONLY).
 *
 * Runs frozen scenario-001..020.md Original Input through the current engines
 * after Sprint 12 room-token identity work.
 *
 * Does NOT modify scenario files, FREEZE_MANIFEST.json, or baseline-validation/.
 *
 * Run: node testing/meridian-gate/sprint12-validation/run-meridian-sprint12-validation.mjs
 *
...
function verifyFreezeManifest() {
  ...
  if (mismatches.length) {
    console.error("FREEZE MANIFEST MISMATCH — aborting Sprint 12 validation run");
...
function runOne({ Engine, Shift, scenario, head, ranAt }) {
  ...
  return {
    ...
    reviewPlaceholder: {
      ...
      note: "Sprint 12 room-token validation only. Compare to frozen Human Expected Truth. Do not modify engine or scenarios.",
    },
  };
}
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(
    `# Scenario-${payload.scenarioId} — MERIDIAN GATE Sprint 12 room-token validation OUTPUT`
  );
  lines.push("");
  lines.push("**Label:** MERIDIAN GATE Sprint 12 room-token validation OUTPUT");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    "Full dump: `scenario-" + payload.scenarioId + "-sprint12.json`"
  );
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  const summary = {
    label: "MERIDIAN GATE Sprint 12 room-token validation SUMMARY",
    gitCommit: head,
    engineVersion: Shift.VERSION,
    contractVersion: Shift.CONTRACT_VERSION || null,
    ranAt,
  ...
  for (let n = 1; n <= 20; n++) {
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-sprint12.json"),
      JSON.stringify(payload, null, 2),
      "utf8"
    );
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-sprint12.md"),
      toMarkdown(payload),
      "utf8"
    );
    ...
    summary.scenarios.push({
      ...
      files: [
        "scenario-" + id + "-sprint12.json",
        "scenario-" + id + "-sprint12.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "MERIDIAN_SPRINT12_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 20/20 Meridian Sprint 12 validation results + MERIDIAN_SPRINT12_SUMMARY.json");
}
