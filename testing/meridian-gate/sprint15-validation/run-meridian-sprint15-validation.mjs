/**
 * Meridian Gate ? Sprint 15 state-resolution completion validation (TEST ONLY).
 *
 * Runs frozen scenario-001..020.md Original Input through the current engines
 * after Sprint 15 soft/mitigated maintenance MONITOR and completed amenity non-reopen work.
 *
 * Does NOT modify scenario files, FREEZE_MANIFEST.json, or baseline-validation/.
 *
 * Run: node testing/meridian-gate/sprint15-validation/run-meridian-sprint15-validation.mjs
 *
...
function verifyFreezeManifest() {
  ...
  if (mismatches.length) {
    console.error("FREEZE MANIFEST MISMATCH ? aborting Sprint 15 validation run");
...
function runOne({ Engine, Shift, scenario, head, ranAt }) {
  ...
  return {
    ...
    reviewPlaceholder: {
      ...
      note: "Sprint 15 state-resolution validation only. Compare to frozen Human Expected Truth. Do not modify engine or scenarios.",
    },
  };
}
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(
    `# Scenario-${payload.scenarioId} ? MERIDIAN GATE Sprint 15 state-resolution validation OUTPUT`
  );
  lines.push("");
  lines.push("**Label:** MERIDIAN GATE Sprint 15 state-resolution validation OUTPUT");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    "Full dump: `scenario-" + payload.scenarioId + "-sprint15.json`"
  );
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  const summary = {
    label: "MERIDIAN GATE Sprint 15 state-resolution validation SUMMARY",
    gitCommit: head,
    engineVersion: Shift.VERSION,
    contractVersion: Shift.CONTRACT_VERSION || null,
    ranAt,
  ...
  for (let n = 1; n <= 20; n++) {
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-sprint15.json"),
      JSON.stringify(payload, null, 2),
      "utf8"
    );
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-sprint15.md"),
      toMarkdown(payload),
      "utf8"
    );
    ...
    summary.scenarios.push({
      ...
      files: [
        "scenario-" + id + "-sprint15.json",
        "scenario-" + id + "-sprint15.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "MERIDIAN_SPRINT15_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 20/20 Meridian Sprint 15 validation results + MERIDIAN_SPRINT15_SUMMARY.json");
}
