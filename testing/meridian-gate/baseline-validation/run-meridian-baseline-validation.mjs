/**
 * Meridian Gate — first-run baseline validation (TEST ONLY).
 *
 * Runs frozen scenario-001..020.md Original Input through the CURRENT committed
 * engines (Sprint 11 HEAD) with no engine changes.
 *
 * Does NOT modify scenario files, FREEZE_MANIFEST.json, HET, or engines.
 *
 * Run: node testing/meridian-gate/baseline-validation/run-meridian-baseline-validation.mjs
 */
import fs from "fs";
...
function verifyFreezeManifest() {
  ...
  if (mismatches.length) {
    console.error("FREEZE MANIFEST MISMATCH — aborting baseline run");
...
function runOne({ Engine, Shift, scenario, head, ranAt }) {
  ...
  return {
    ...
    reviewPlaceholder: {
      ...
      note: "First-run baseline only. Compare to frozen Human Expected Truth. Do not modify engine or scenarios.",
    },
  };
}
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(
    `# Scenario-${payload.scenarioId} — MERIDIAN GATE FIRST-RUN BASELINE OUTPUT`
  );
  lines.push("");
  lines.push("**Label:** MERIDIAN GATE FIRST-RUN BASELINE OUTPUT");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    "Full dump: `scenario-" + payload.scenarioId + "-baseline.json`"
  );
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  const summary = {
    label: "MERIDIAN GATE FIRST-RUN BASELINE SUMMARY",
    gitCommit: head,
    engineVersion: Shift.VERSION,
    contractVersion: Shift.CONTRACT_VERSION || null,
    ranAt,
  ...
  for (let n = 1; n <= 20; n++) {
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-baseline.json"),
      JSON.stringify(payload, null, 2),
      "utf8"
    );
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-baseline.md"),
      toMarkdown(payload),
      "utf8"
    );
    ...
    summary.scenarios.push({
      ...
      files: [
        "scenario-" + id + "-baseline.json",
        "scenario-" + id + "-baseline.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "MERIDIAN_BASELINE_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 20/20 Meridian baseline results + MERIDIAN_BASELINE_SUMMARY.json");
}
