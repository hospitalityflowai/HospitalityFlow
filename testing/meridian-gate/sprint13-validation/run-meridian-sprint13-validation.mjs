/**
 * Meridian Gate ù Sprint 13 invalid inventory / product validation (TEST ONLY).
 *
 * Runs frozen scenario-001..020.md Original Input through the current engines
 * after Sprint 13 invalid inventory / invalid product configuration work.
 *
 * Does NOT modify scenario files, FREEZE_MANIFEST.json, or baseline-validation/.
 *
 * Run: node testing/meridian-gate/sprint13-validation/run-meridian-sprint13-validation.mjs
 *
...
function verifyFreezeManifest() {
  ...
  if (mismatches.length) {
    console.error("FREEZE MANIFEST MISMATCH ù aborting Sprint 13 validation run");
...
function runOne({ Engine, Shift, scenario, head, ranAt }) {
  ...
  return {
    ...
    pipeline: [
      ...
      "buildCanonicalOperationalActions (Sprint 5/6/8/9/10/11 ù current HEAD)",
    ...
    reviewPlaceholder: {
      ...
      note: "Sprint 13 invalid-inventory validation only. Compare to frozen Human Expected Truth. Do not modify engine or scenarios.",
    },
  };
}
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(
    `# Scenario-${payload.scenarioId} ù MERIDIAN GATE Sprint 13 invalid-inventory validation OUTPUT`
  );
  lines.push("");
  lines.push("**Label:** MERIDIAN GATE Sprint 13 invalid-inventory validation OUTPUT");
  lines.push("");
  lines.push(
  ...
  lines.push(`- Scenario: ${payload.scenarioId} ù ${payload.title}`);
  ...
  if (!acts.length) lines.push("_No canonical actions_");
    ...
    acts.forEach((a) => {
      const tempBits = [a.temporalScope, a.serviceDate, a.deadlineHint]
        ...
        .join("/");
      lines.push(
        `- **${a.actionState}** \`${a.facetKey}\` ${a.priorityBand || ""} ù ${a.actionText}` +
  ...
  lines.push("");
  lines.push(
    "Full dump: `scenario-" + payload.scenarioId + "-sprint13.json`"
  );
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  const summary = {
    label: "MERIDIAN GATE Sprint 13 invalid-inventory validation SUMMARY",
    gitCommit: head,
    engineVersion: Shift.VERSION,
    contractVersion: Shift.CONTRACT_VERSION || null,
    ranAt,
  ...
  for (let n = 1; n <= 20; n++) {
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-sprint13.json"),
      JSON.stringify(payload, null, 2),
      "utf8"
    );
    fs.writeFileSync(
      path.join(OUT_DIR, "scenario-" + id + "-sprint13.md"),
      toMarkdown(payload),
      "utf8"
    );
    ...
    summary.scenarios.push({
      ...
      files: [
        "scenario-" + id + "-sprint13.json",
        "scenario-" + id + "-sprint13.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "MERIDIAN_SPRINT13_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 20/20 Meridian Sprint 13 validation results + MERIDIAN_SPRINT13_SUMMARY.json");
}
