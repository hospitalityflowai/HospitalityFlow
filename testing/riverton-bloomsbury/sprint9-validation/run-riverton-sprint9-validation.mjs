/**
 * Riverton Bloomsbury — Sprint 9 payment fail-closed validation (TEST ONLY).
 *
 * Re-runs approved scenario-001..020.md Original Input unchanged through the
 * current engines (post Sprint 9 payment collect harden) into this folder.
 *
 * Does NOT overwrite sprint8-baseline-validation/ artefacts or scenario files.
 *
 * Run: node testing/riverton-bloomsbury/sprint9-validation/run-riverton-sprint9-validation.mjs
 */
import fs from "fs";
...
function runOne({ Engine, Shift, scenario, head, ranAt }) {
  ...
  return {
    ...
    reviewPlaceholder: {
      ...
      note: "Compare to sprint8-baseline-validation/; payment fail-closed is the Sprint 9 focus.",
    },
  };
}
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(
    `# Scenario-${payload.scenarioId} — Riverton Sprint 9 VALIDATION OUTPUT`
  );
  lines.push("");
  lines.push("**Label:** RIVERTON SPRINT 9 VALIDATION OUTPUT");
  lines.push("");
  lines.push(
...
function main() {
  ...
  for (let n = 1; n <= 20; n++) {
    ...
    const jsonPath = path.join(OUT_DIR, "scenario-" + id + "-sprint9.json");
    const mdOutPath = path.join(OUT_DIR, "scenario-" + id + "-sprint9.md");
    ...
    summary.scenarios.push({
      ...
      files: [
        "scenario-" + id + "-sprint9.json",
        "scenario-" + id + "-sprint9.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "RIVERTON_SPRINT9_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log(
    "\nWrote 20/20 Riverton Sprint 9 validation results + RIVERTON_SPRINT9_SUMMARY.json"
  );
}
