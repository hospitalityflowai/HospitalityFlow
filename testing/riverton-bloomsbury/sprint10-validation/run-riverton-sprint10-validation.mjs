/**
 * Riverton Bloomsbury — Sprint 10 state-resolution validation (TEST ONLY).
 *
 * Re-runs approved scenario-001..020.md Original Input unchanged through the
 * current engines (post Sprint 10 state-resolution harden) into this folder.
 *
 * Does NOT overwrite sprint8-baseline-validation / sprint9-validation/ artefacts or scenario files.
 *
 * Run: node testing/riverton-bloomsbury/sprint10-validation/run-riverton-sprint10-validation.mjs
 */
import fs from "fs";
...
function runOne({ Engine, Shift, scenario, head, ranAt }) {
  ...
  return {
    ...
    reviewPlaceholder: {
      ...
      note: "Compare to sprint8-baseline-validation / sprint9-validation/; state-resolution harden is the Sprint 10 focus. Compare to frozen sprint9-validation/.",
    },
  };
}
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(
    `# Scenario-${payload.scenarioId} — RIVERTON SPRINT 10 VALIDATION OUTPUT`
  );
  lines.push("");
  lines.push("**Label:** RIVERTON SPRINT 10 VALIDATION OUTPUT");
  lines.push("");
  lines.push(
...
function main() {
  ...
  for (let n = 1; n <= 20; n++) {
    ...
    const jsonPath = path.join(OUT_DIR, "scenario-" + id + "-sprint10.json");
    const mdOutPath = path.join(OUT_DIR, "scenario-" + id + "-sprint10.md");
    ...
    summary.scenarios.push({
      ...
      files: [
        "scenario-" + id + "-sprint10.json",
        "scenario-" + id + "-sprint10.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "RIVERTON_SPRINT10_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log(
    "\nWrote 20/20 RIVERTON SPRINT 10 validation results + RIVERTON_SPRINT10_SUMMARY.json"
  );
}
