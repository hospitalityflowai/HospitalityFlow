# Scenario-001 â RIVERTON SPRINT 11 VALIDATION OUTPUT
...
## Run metadata
- Scenario: 001 â Quiet Monday Continuity
- Shift: AM
- Load: Quiet
- Difficulty: Basic
...
- State counts: {"open":0,"monitor":0,"information":2,"unresolved":0,"blocked":0,"resolved":3,"other":0}

- **information** `vip:prep_complete` P3 â Mrs Delgado — VIP prep complete / awareness only _(room 405)_ _(guest Mrs Delgado)_
- **information** `timed:late_checkout_not_requested` P3 â Late check-out not requested for Room 308 — standard departure _(room 308)_ _(guest Ms Kaur)_
- **resolved** `guest_request:extra_pillows:done` exclude â extra pillows already completed for Room 119 _(room 119)_ _(guest - Guest)_
- **resolved** `payment:no_collect` exclude â Payment settled / no collect required for Room 214 _(room 214)_ _(guest Mr Ellison)_
- **resolved** `guest_request` exclude â Superseded current-state fact _(room 405)_ _(guest Mrs Delgado)_
...
1. Confirm the late check-out for Room 308 and advise Housekeeping of the release time — late check-out still needs confirmation before Housekeeping release. _(priority: normal, owner: Housekeeping, status: open)_
2. Log and follow up lost item with Reception this shift. _(priority: normal, owner: Reception, status: open)_
...
Full machine-readable dump: `scenario-001-sprint11.json`
