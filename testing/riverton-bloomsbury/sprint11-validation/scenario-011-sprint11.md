# Scenario-011 â RIVERTON SPRINT 11 VALIDATION OUTPUT
...
## Run metadata
- Scenario: 011 â Corporate Block vs Leisure VIP Same Surname
- Shift: PM
- Load: Busy
- Difficulty: Hard
...
- State counts: {"open":2,"monitor":0,"information":3,"unresolved":0,"blocked":0,"resolved":3,"other":0}

- **open** `arrival_prep:high_touch` P1 â Prepare arrival amenities — fruit, welcome card
- **open** `amenity:prep` P3 â Prepare fruit
- **information** `vip:no_active_amenity` P3 â Mr Owen Brennan — No active VIP amenity outstanding _(room 312)_ _(guest Mr Owen Brennan)_
- **information** `vip:no_active_amenity` P3 â No active VIP amenity outstanding
- **information** `payment:insufficient_evidence` P3 â Payment-related note retained — insufficient evidence for collect chase
- **resolved** `superseded` exclude â Superseded current-state fact _(room 312)_ _(guest Mr Owen Brennan)_
- **resolved** `superseded` exclude â Superseded current-state fact _(room 312)_ _(guest Mr Owen Brennan)_
- **resolved** `guest_request` exclude â Superseded current-state fact
...
3. Prepare arrival amenities — fruit, welcome card. _(priority: high, owner: Housekeeping, status: open)_
...
Full machine-readable dump: `scenario-011-sprint11.json`
