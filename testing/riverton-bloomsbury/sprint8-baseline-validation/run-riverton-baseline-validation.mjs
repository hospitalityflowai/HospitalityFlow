/**
 * Riverton Bloomsbury — Sprint 8 baseline validation (TEST ONLY).
 *
 * Reads approved scenario-001..020.md Original Input → current Sprint 8 engines
 * → frozen baseline artefacts in this folder.
 *
 * Does NOT modify engines, scenario Human Expected Truth, Zetter/Pilot evidence,
 * or Sprint 1–8 unit tests.
 *
 * Run: node testing/riverton-bloomsbury/sprint8-baseline-validation/run-riverton-baseline-validation.mjs
 */
import fs from "fs";
...
function runOne({ Engine, Shift, scenario, head, ranAt }) {
  ...
  return {
    ...
    reviewPlaceholder: {
      ...
      note: "Verdicts live in RIVERTON_BASELINE_FAILURE_MAP.md after assessment against Human Expected Truth.",
    },
  };
}
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(
    `# Scenario-${payload.scenarioId} — Riverton Sprint 8 BASELINE OUTPUT`
  );
  lines.push("");
  lines.push("**Label:** RIVERTON SPRINT 8 BASELINE OUTPUT");
  lines.push("");
  lines.push(
...
function main() {
  ...
  for (let n = 1; n <= 20; n++) {
    ...
    const jsonPath = path.join(OUT_DIR, "scenario-" + id + "-baseline.json");
    const mdOutPath = path.join(OUT_DIR, "scenario-" + id + "-baseline.md");
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "RIVERTON_BASELINE_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log(
    "\nWrote 20/20 Riverton Sprint 8 baseline results + RIVERTON_BASELINE_SUMMARY.json"
  );
}
