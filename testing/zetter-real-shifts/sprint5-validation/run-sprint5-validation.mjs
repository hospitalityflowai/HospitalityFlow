/**
 * Controlled Zetter Sprint 5 validation rerun (TEST ONLY).
 * Same historical source_notes → Sprint 5 engines → SPRINT 5 VALIDATION OUTPUT.
 ...
 *
 * Run: node testing/zetter-real-shifts/sprint5-validation/run-sprint5-validation.mjs
 */
import fs from "fs";
...
function runOne({ Engine, Shift, shift, record, sourceFromMd, head, ranAt }) {
  ...
  const canonicalActions = Shift.buildCanonicalOperationalActions
    ? Shift.buildCanonicalOperationalActions(analyzed)
    : [];
  ...
  const payload = {
    ...
    pipeline: [
      ...
      "buildCanonicalOperationalActions (Sprint 5)",
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(`# Shift-${payload.shift} — SPRINT 5 VALIDATION OUTPUT`);
  lines.push("");
  lines.push("**Label:** SPRINT 5 VALIDATION OUTPUT");
  lines.push("");
  ...
  lines.push("```");
  lines.push("");
  lines.push("## Canonical actions (Sprint 5)");
  lines.push("");
  ...
  lines.push("");
  lines.push("Full machine-readable dump: `shift-" + payload.shift + "-sprint5.json`");
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  for (const { shift, id } of MAPPING) {
    ...
    const jsonPath = path.join(OUT_DIR, "shift-" + shift + "-sprint5.json");
    const mdOutPath = path.join(OUT_DIR, "shift-" + shift + "-sprint5.md");
    ...
    summary.shifts.push({
      ...
      files: [
        "shift-" + shift + "-sprint5.json",
        "shift-" + shift + "-sprint5.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "SPRINT5_VALIDATION_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 5/5 SPRINT 5 VALIDATION results + SPRINT5_VALIDATION_SUMMARY.json");
}
