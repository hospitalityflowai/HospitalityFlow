/**
 * Controlled Zetter Sprint 7 validation rerun (TEST ONLY).
 * Same historical source_notes → Sprint 7 engines → Sprint 7 VALIDATION OUTPUT.
 * Does NOT overwrite historical or post-Sprint-4 benchmark artefacts.
 *
 * Pipeline:
 *   extract → consolidate → entities → current-state → dependencies
 *   → buildCanonicalOperationalActions → organised → briefing → recommendations
 *
 * Run: node testing/zetter-real-shifts/sprint7-validation/run-sprint7-validation.mjs
 */
import fs from "fs";
...
function runOne({ Engine, Shift, shift, record, sourceFromMd, head, ranAt }) {
  ...
  const temporalOpts = {
    handoverDate: record.handover_date || "",
    shift: record.shift || "Night",
  ...
  /* Sprint 7: one shared canonical list for Briefing + Recommendations parity. */
  analyzed._canonicalActions = canonicalActions;
  analyzed._canonicalActionsBuilt = true;

  const briefing = Engine.buildTodaysBriefing
    ? Engine.buildTodaysBriefing(analyzed, Object.assign({ maxBlocks: 5 }, temporalOpts, {
  ...
  const payload = {
    ...
    pipeline: [
      ...
      "buildCanonicalOperationalActions (Sprint 5/6)",
      "buildOrganisedSectionModel",
      "buildTodaysBriefing (Sprint 7 decision seating)",
      "ShiftIntelligenceEngine.analyze (Sprint 7 recommendation seating)",
    ],
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(`# Shift-${payload.shift} — Sprint 7 VALIDATION OUTPUT`);
  lines.push("");
  lines.push("**Label:** Sprint 7 VALIDATION OUTPUT");
  lines.push("");
  lines.push("Do **not** confuse with HISTORICAL HF OUTPUT or post-Sprint-4 `current-engine-rerun/`.");
  lines.push("");
  ...
  lines.push("```");
  lines.push("");
  lines.push("## Canonical actions (Sprint 7 + temporal)");
  lines.push("");
  ...
  lines.push("");
  lines.push("Full machine-readable dump: `shift-" + payload.shift + "-sprint7.json`");
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  for (const { shift, id } of MAPPING) {
    ...
    const jsonPath = path.join(OUT_DIR, "shift-" + shift + "-sprint7.json");
    const mdOutPath = path.join(OUT_DIR, "shift-" + shift + "-sprint7.md");
    fs.writeFileSync(jsonPath, JSON.stringify(payload, null, 2), "utf8");
    fs.writeFileSync(mdOutPath, toMarkdown(payload), "utf8");

    const openCount = (payload.canonicalActions || []).filter(
      (a) => a.actionState === "open"
    ).length;
    summary.shifts.push({
      shift,
      recordId: id,
      ...
      openCanonicalActionCount: openCount,
      briefingChars: (payload.aiSummaryBriefing || "").length,
      files: [
        "shift-" + shift + "-sprint7.json",
        "shift-" + shift + "-sprint7.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "SPRINT7_VALIDATION_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 5/5 Sprint 7 VALIDATION results + SPRINT7_VALIDATION_SUMMARY.json");
}
