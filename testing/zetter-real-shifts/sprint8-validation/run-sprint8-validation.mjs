/**
 * Controlled Zetter Sprint 8 validation rerun (TEST ONLY).
 * Same historical source_notes → Sprint 8 engines → Sprint 8 VALIDATION OUTPUT.
 ...
 *
 * Run: node testing/zetter-real-shifts/sprint8-validation/run-sprint8-validation.mjs
 */
import fs from "fs";
...
function runOne({ Engine, Shift, shift, record, sourceFromMd, head, ranAt }) {
  ...
  /* Sprint 8: one shared canonical list for Briefing + Recommendations parity. */
  ...
  const payload = {
    ...
    pipeline: [
      ...
      "buildCanonicalOperationalActions (Sprint 5/6/8)",
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(`# Shift-${payload.shift} — Sprint 8 VALIDATION OUTPUT`);
  lines.push("");
  lines.push("**Label:** Sprint 8 VALIDATION OUTPUT");
  lines.push("");
  ...
  lines.push("");
  lines.push("Full machine-readable dump: `shift-" + payload.shift + "-sprint8.json`");
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  for (const { shift, id } of MAPPING) {
    ...
    const jsonPath = path.join(OUT_DIR, "shift-" + shift + "-sprint8.json");
    const mdOutPath = path.join(OUT_DIR, "shift-" + shift + "-sprint8.md");
    ...
    summary.shifts.push({
      ...
      files: [
        "shift-" + shift + "-sprint8.json",
        "shift-" + shift + "-sprint8.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "SPRINT8_VALIDATION_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 5/5 Sprint 8 VALIDATION results + SPRINT8_VALIDATION_SUMMARY.json");
}
