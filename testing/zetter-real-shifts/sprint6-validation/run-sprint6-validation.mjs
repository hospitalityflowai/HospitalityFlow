/**
 * Controlled Zetter Sprint 6 validation rerun (TEST ONLY).
 * Same historical source_notes → Sprint 6 engines → Sprint 6 VALIDATION OUTPUT.
 ...
 *
 * Run: node testing/zetter-real-shifts/sprint6-validation/run-sprint6-validation.mjs
 */
import fs from "fs";
...
function runOne({ Engine, Shift, shift, record, sourceFromMd, head, ranAt }) {
  ...
  const payload = {
    ...
    pipeline: [
      ...
      "buildCanonicalOperationalActions (Sprint 6)",
...
function toMarkdown(payload) {
  const lines = [];
  lines.push(`# Shift-${payload.shift} — Sprint 6 VALIDATION OUTPUT`);
  lines.push("");
  lines.push("**Label:** Sprint 6 VALIDATION OUTPUT");
  lines.push("");
  ...
  lines.push("```");
  lines.push("");
  lines.push("## Canonical actions (Sprint 6 + temporal)");
  lines.push("");
  ...
  lines.push("");
  lines.push("Full machine-readable dump: `shift-" + payload.shift + "-sprint6.json`");
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  for (const { shift, id } of MAPPING) {
    ...
    const jsonPath = path.join(OUT_DIR, "shift-" + shift + "-sprint6.json");
    const mdOutPath = path.join(OUT_DIR, "shift-" + shift + "-sprint6.md");
    ...
    summary.shifts.push({
      ...
      files: [
        "shift-" + shift + "-sprint6.json",
        "shift-" + shift + "-sprint6.md",
      ],
    });
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, "SPRINT6_VALIDATION_SUMMARY.json"),
    JSON.stringify(summary, null, 2),
    "utf8"
  );
  console.log("\nWrote 5/5 Sprint 6 VALIDATION results + SPRINT6_VALIDATION_SUMMARY.json");
}
